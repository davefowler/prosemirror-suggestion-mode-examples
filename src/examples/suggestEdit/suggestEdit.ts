...
window.addEventListener("load", () => {
  ...
  const content = `
    <p>We choose to go to the Moon in this decade and do the other things, not because they are easy, but because they are hard, because that goal will serve to organise and measure the best of our energies and skills, because that challenge is one that we are willing to accept, one we are unwilling to postpone, and one which we intend to win, and the others, too.</p>
  ...
  const exampleSuggestions: TextSuggestion[] = [
    ...
    //   prefix: "will serve to organi",
    //   suffix: "e and measure",
    // },
  ];
  ...
  exampleSuggestions.forEach((suggestion, index) => {
    ...
    if (suggestion.prefix) {
      formattedText += `    prefix: "${suggestion.prefix}",\n`;
    }

    if (suggestion.suffix) {
      formattedText += `    suffix: "${suggestion.suffix}",\n`;
    }
