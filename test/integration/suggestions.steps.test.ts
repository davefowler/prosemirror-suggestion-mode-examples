...
describe("suggestion mode edge cases", () => {
  ...
  function createEditor(content: string = "<p>Hello world</p>", pluginState = {}) {
    ...
    state = EditorState.create({
      doc,
      schema,
      plugins: [keymap(baseKeymap), suggestionModePlugin({ username: "test user" })],
    });
  ...
  describe("pasting into suggestion range", () => {
    test("should handle pasting text into an existing suggestion_add range", () => {
      ...
      view.state.doc.nodesBetween(position, position + "awePASTEDsome".length, (node) => {
    ...
    test("should handle pasting text into an existing suggestion_delete range", () => {
      ...
      let hasAddMark = false;
      view.state.doc.nodesBetween(from, from + "awesome PASTED".length, (node) => {
  ...
  describe("deletion followed by typing", () => {
    test("should handle backspace followed by typing", () => {
      ...
      let hasDeleteMark = false;
      let hasAddMark = false;
      view.state.doc.nodesBetween(from, from + "awesome wonderful".length, (node) => {
        node.marks.forEach(mark => {
          if (mark.type.name === "suggestion_delete") hasDeleteMark = true;
          if (mark.type.name === "suggestion_add") hasAddMark = true;
        });
  ...
  describe("copying and pasting formatted text", () => {
    test("should handle pasting pre-formatted text", () => {
      ...
      const position = 6;
      view.dispatch(
        view.state.tr.setSelection(Selection.near(view.state.doc.resolve(position)))
      );
      ...
      const pastedFragment = DOMParser.fromSchema(schema).parse(tempDiv).content;

      view.dispatch(view.state.tr.replaceWith(position, position, pastedFragment));
      ...
      let hasAddMark = false;
      let hasStrongMark = false;
      view.state.doc.nodesBetween(position, position + "pasted".length, (node) => {
        node.marks.forEach(mark => {
          if (mark.type.name === "suggestion_add") hasAddMark = true;
          if (mark.type.name === "strong") hasStrongMark = true;
        });
