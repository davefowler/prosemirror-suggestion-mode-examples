...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const editorState = EditorState.create({
      ...
      plugins: [
        ...
        keymap({
          ...
          'Shift-Ctrl-1': setBlockType(advancedSchema.nodes.heading, {
            level: 1,
          }),
          'Shift-Ctrl-2': setBlockType(advancedSchema.nodes.heading, {
            level: 2,
          }),
          'Shift-Ctrl-3': setBlockType(advancedSchema.nodes.heading, {
            level: 3,
          }),
        }),
        keymap(baseKeymap),
  ...
  function updateToolbarState(view: EditorView) {
    ...
    // Update bold button
    const boldActive =
      schema.marks.strong.isInSet(state.doc.resolve(from).marks()) ||
    ...
    const italicActive =
      schema.marks.em.isInSet(state.doc.resolve(from).marks()) ||
      (!empty && state.doc.rangeHasMark(from, to, schema.marks.em));
    document
      .getElementById('italic')
      .classList.toggle('active', !!italicActive);
  ...
  document.getElementById('h1').addEventListener('click', () => {
    setBlockType(advancedSchema.nodes.heading, { level: 1 })(
      view.state,
      view.dispatch,
      view
    );
  ...
  document.getElementById('h2').addEventListener('click', () => {
    setBlockType(advancedSchema.nodes.heading, { level: 2 })(
      view.state,
      view.dispatch,
      view
    );
  ...
  document.getElementById('h3').addEventListener('click', () => {
    setBlockType(advancedSchema.nodes.heading, { level: 3 })(
      view.state,
      view.dispatch,
      view
    );
